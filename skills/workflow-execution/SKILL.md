...
description: Use before recipe/table writes, multi-step table work, widening, validation reads, and async/polling columns — the durable execution lifecycle, path lock, and post-write check over remote MCP.
---
...
- **Recipe writes are real commits, not probes.** `copilot_create_recipe` and `copilot_workflow_workbench` writes run immediately over MCP — there is no approval card. Plan the workflow in the conversation and get the user's explicit go-ahead before the first `copilot_create_recipe`.
...
- **Fix bad columns at the bad column.** Provider pacing is ingredient config: never invent rate/concurrency; after docs/headers or observed attempt-rate evidence isolates pacing, update the ingredient before retrying or widening.
...
**Same-path proof or a committed user-facing column locks the route** for that value. Do not replace a locked provider/tool/source because another looks cheaper, cleaner, easier, or faster. Switch only when the locked route has an actual failure point and cannot continue, the user approves, or the new branch is an explicit conditional fallback. Proof must match the path you will wire: a saved action/ingredient needs `copilot_workflow_workbench(action:"external_call")` proof for that exact path.

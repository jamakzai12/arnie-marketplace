...
description: Use for copilot_create_recipe or complex copilot_workflow_workbench authoring over remote MCP — row shape, seed rows, formula inputs, pagination, arrays, and modify rules.
---
...
`copilot_create_recipe` runs immediately over MCP — there is no approval card and nothing pauses for sign-off. Lay out the whole workflow in the conversation (tables, columns, providers, and the spend it implies) and get the user's explicit go-ahead. Only then:

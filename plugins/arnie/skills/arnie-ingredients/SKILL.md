...
description: Use the Arnie CLI for saved ingredients, provider configs, API credentials, reusable API/tool contracts, or ingredient-backed workflow columns — saving, updating, rate-limit repair, and the credential flow.
---
...
Preserve existing rate-limit and concurrency settings. Never add or change them from provider docs, headers, references, or preventative guesses. Only actual table cells showing provider rate-limit failures allow a repair; then add or lower pacing only for the failing ingredient with `copilot_update_ingredient`. Let scheduled retries run or rerun once after the fix instead of hammering manual reruns.

## Credentials

- Connect API-key and OAuth credentials in the Arnie web app under Settings → Integrations. After the user connects the provider, run `copilot_search_workspace` again and continue with the saved connection or returned `credentialId`.
- **Never ask the user to paste API keys, tokens, cookies, passwords, or auth headers in chat, and never pass them in `arnie call` arguments.** Shell arguments can be read by other local processes. Let Arnie's browser UI collect and store secrets safely.
